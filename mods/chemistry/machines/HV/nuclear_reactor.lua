--[[
This file is only used to override the run code for the
nuclear reactor to allow the insertion of Plutonium Fuel.
--]]

local burn_ticks = 7 * 24 * 60 * 60  -- Seconds
local power_supply = 100000  -- EUs
local fuel_type_uranium = "technic:uranium_fuel"  -- The reactor burns this
local fuel_type_plutonium = "chemistry:plutonium_fuel"
...
local function start_reactor(pos, meta)
	...
	local msg_fuel_missing = "Error: You need to insert " .. correct_fuel_count .. " pieces of Uranium Fuel or Plutonium Fuel."
	...
	for _, src_stack in pairs(src_list) do
		if src_stack and src_stack:get_name() == fuel_type_uranium then
			fuel_count = fuel_count + 1
		elseif src_stack and src_stack:get_name() == fuel_type_plutonium then
			fuel_count = fuel_count + 1
		end
	end
...
minetest.override_item("technic:hv_nuclear_reactor_core_active", {
	technic_run = run,
})
