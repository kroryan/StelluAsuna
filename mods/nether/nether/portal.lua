...
if nether.trap_players then
	function update_background(player, down)
		if down then
			player:set_sky({type="plain", base_color="#0F0000"})
		else
			player:set_sky()
		end
	end
...
local function make_portal(pos)
	...
	for y=p1.y,p2.y do
		...
		meta:mark_as_private({"p1", "p2", "target"})
	end
	end
...
-- Remove nether:portal nodes and obsidian node metadata of a MineCraft-like
-- portal and its opposite portal if they exist
local function destroy_mc_portals(pos)
	local meta = minetest.get_meta(pos)
...
local obsidian_destruct_old = minetest.registered_nodes[
	"default:obsidian"].on_destruct or function() end
minetest.override_item("default:obsidian", {
	on_destruct = function(pos, ...)
		destroy_mc_portals(pos)
		return obsidian_destruct_old(pos, ...)
	end
})

-- Override mese crystal fragment for making an obsidian portal
local old_mesefragment_on_place = minetest.registered_items[
	"default:mese_crystal_fragment"].on_place or minetest.item_place
minetest.override_item("default:mese_crystal_fragment", {
	on_place = function(stack, player, pt, ...)
		...
		return old_mesefragment_on_place(stack, player, pt, ...)
	end
})
