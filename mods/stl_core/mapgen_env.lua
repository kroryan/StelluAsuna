...
minetest.register_on_generated(function(_, minp, maxp)
	if maxp.y < 6368 then return end
	...
	-- Keep Stellua's geology and add Asuna's dominant surface palette and
	-- occasional ground flora on living planets.
	local mix = stellua.asuna_mix
	if mix then
		for y = minp.y, maxp.y do
			local index, planet = unpack(y_planets[y])
			if planet and planet.life_stat > 0.5 then
				local planet_noise = noises2d["planet"..index]
				local river_noise = noises2d["river"..index]
				local rand = PcgRandom(planet.seed + minp.x * 7349 + minp.z * 9151 + y)
				for x = minp.x, maxp.x do for z = minp.z, maxp.z do
					local ni = sl * (z - minp.z) + (x - minp.x) + 1
					local surface = planet_noise.data[ni]
					if planet.water_level then
						surface = min(surface, river_noise.data[ni]^2 + planet.river_level)
					end
					local sy = math.floor(surface) - 1
					if sy == y and sy >= emin.y and sy + 1 <= emax.y then
						local vi = area:index(x, sy, z)
						local above = area:index(x, sy + 1, z)
						local ground = data[vi]
						local is_surface = ground == planet.c_filler or ground == planet.c_beach
							or ground == planet.c_seabed
						if is_surface and data[above] == c_air and rand:next(1, 100) <= 65 then
							local surfaces = planet.heat_stat < 240 and mix.cold_surfaces
								or planet.heat_stat > 360 and mix.dry_surfaces
								or mix.warm_surfaces
							if #surfaces > 0 then
								data[vi] = surfaces[rand:next(1, #surfaces)]
								if #mix.plants > 0 and rand:next(1, 100) <= 14 then
									data[above] = mix.plants[rand:next(1, #mix.plants)]
								end
							end
						end
					end
