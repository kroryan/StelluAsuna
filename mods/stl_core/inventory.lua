...
sfinv.register_page("stl_core:planets", {
    ...
    get = function (self, player, context)
        ...
        if context.planet == 0 then
            local x, y = 0, 1.1
            table.insert(out, "button[0,0;4,1;homeworld;Return to Asuna homeworld]")
        ...
        elseif context.planet < 0 then
            ...
            if slot then
                ...
                -- get_slot_info returns nil, nil if `slot` has no stored
                -- entry (a non-nil slot index doesn't guarantee a hit) --
                -- indexing stellua.stars[nil].pos below used to crash the
                -- whole server with "attempt to index a nil value" every
                -- time a player opened this menu without a valid slot.
                if star and pos then
    ...
    on_player_receive_fields = function (self, player, context, fields)
        ...
        for i = 1, stellua.planet_count do
