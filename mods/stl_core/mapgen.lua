...
local PLANET_COUNT = 23
local STAR_COUNT = 8
local PLANET_BASE = 7000
stellua.hybrid_space_min = 6368
stellua.planet_count = PLANET_COUNT
stellua.star_count = STAR_COUNT
...
function stellua.get_planet_index(y)
	-- Keep these constants literal: Luanti copies this function into the
	-- mapgen environment, where ordinary Lua upvalues are not preserved.
	local index = math.round((y-7000)*0.001)
	if index < 1 or index > 23 then return end
    return index
end
...
-- Hybrid mode intentionally keeps Asuna's v7 mapgen. mapgen_env.lua only
-- replaces aligned chunks in the reserved high-altitude space band.
