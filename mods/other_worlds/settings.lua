...
otherworlds.settings.space_asteroids = {
	...
	YMAX = tonumber(core.settings:get("otherworlds.space.ymax") or 5999)
}
...
otherworlds.settings.redsky_asteroids = {
	...
	YMAX = tonumber(core.settings:get("otherworlds.redsky.ymax") or 6999)
}

-- blackness (deep space)

otherworlds.settings.blackness = {
	-- set to false to prevent blackness mapgen
	enable = core.settings:get_bool("otherworlds.blackness", true),
	-- minimum height of blackness layer
	YMIN = tonumber(core.settings:get("otherworlds.blackness.ymin") or 7000),
	-- maximum height for blackness layer
	YMAX = tonumber(core.settings:get("otherworlds.blackness.ymax") or 31000)
}
