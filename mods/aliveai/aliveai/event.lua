...
aliveai.searchobjects=function(self)
		...
		-- invalidated earlier in the same server step -- passing that nil
		-- straight into get_objects_inside_radius() below used to crash
		-- the whole server ("Invalid vector (expected table got nil)"),
		-- same root-cause class as the get_velocity()/get_pos() crashes
		-- fixed in bot.lua and base.lua.
