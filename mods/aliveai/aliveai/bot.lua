...
on_punch=function(self, puncher, time_from_last_punch, tool_capabilities, dir)
		...
		if dir~=nil and mindmg==true and self.object then
			-- self.object:get_velocity() can return nil if this entity was
			-- already invalidated earlier in the same server step (e.g. by
			-- aliveai_threats combat logic) -- indexing .y on that nil used
			-- to crash the whole server (bot.lua:255 "attempt to index a nil
			-- value"). Guard both this immediate use and the delayed callback.
			local vel = self.object:get_velocity()
			if vel then
				local v={x = dir.x*3,y = vel.y,z = dir.z*3}
