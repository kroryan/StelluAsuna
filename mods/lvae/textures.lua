			...
			for _, tile in pairs(def[ttype]) do
				if type(tile) == "table" then
					...
					if tile.animation and not sheets[tile.name] then
						-- Texture modifiers (^[colorize:..., etc) don't change base image
						-- dimensions, but they do break a plain-filename lookup against
						-- mods[...]/all_textures (both keyed by bare filenames) -- confirmed
						-- the hard way with digtron's "digtron_digger_yb.png^[colorize:...]"
						-- animated tile, which otherwise crashed lvae's tilesheet sizing here.
						local base_name = tile.name:match("^([^%^]+)") or tile.name
						local resolved = mods[def.mod_origin] and mods[def.mod_origin][base_name] or all_textures[base_name]
						if not resolved then
							minetest.log("warning", "[lvae] skipping tilesheet sizing for unresolved animated texture: node=" .. tostring(def.name) .. " tile=" .. tostring(tile.name))
						else
						-- Could just read the image and extract the proper headers, but too lazy to do that
						local image = png.load_from_file(resolved)
						local w, h = 0, 0
